...
usage() {
  cat <<'EOF'
Usage: ./scripts/start_web_with_ros.sh [launcher options] [web_ui.py options]
...
Use ./scripts/start_web_no_ros.sh for dry-run Web testing.
  ...
  while (($#)); do
    case "$1" in
      ...
      --dry-run-ros2)
        ...
        echo "[ai-ov] Use ./scripts/start_web_no_ros.sh for dry-run Web testing." >&2
        exit 2
        ;;
      -h|--help)
        usage
